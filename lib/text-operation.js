...
ot.TextOperation = (function () {
  ...
  TextOperation.transform = function (operation1, operation2) {
    ...
    while (true) {
      ...
      if (isInsert(op1)) {
        // op1是插入操作 operation1prime执行插入操作
        operation1prime.insert(op1);
        // operation2prime skip op1.length位,确保两个操作在 inputString的同一位置
        operation2prime.retain(op1.length);

        console.log(operation1prime.length , operation2prime.length);
        // ops1的指向下一个op(注意ops2还没处理)
        op1 = ops1[i1++];
        continue;
      }
      if (isInsert(op2)) {
        // 如果op1不是插入, 而op2是插入的情况, 同上
      ...
      if (isRetain(op1) && isRetain(op2)) {
        ...
        if (op1 > op2) {
          minl = op2;
          op1 = op1 - op2; // op1 剩下的retain(op1 - op2)在下一轮循环中进行处理
          op2 = ops2[i2++]; 
        } else if (op1 === op2) {
          minl = op2;
          op1 = ops1[i1++];
        ...
        // operation1prime 和 operation2prime同时后移
        ...
        // 如果都是删除，先删除 删除字符少的 min(abs(op1), abs(op2)), 剩下的留到下一轮和下一个操作一起处理
        if (-op1 > -op2) {
          // op1 删除的多
          op1 = op1 - op2;
          op2 = ops2[i2++];
